...
-   Взвешенная общая точность. Именно она отображается в профиле, и расчитывается по тому же принципу, что и [PP](/wiki/Performance_Points): влияние точности отдельно взятого результата прямо пропорционально его весу (указано на сайте как "weighted **X%**"). 
...
### Взвешенная общая точность (для всех режимов)

Как и простая точность, вычисляется как отношение полученной игроком точности к максимально возможной точности. Отличие только в том, что каждый компонент как делимого, так и делителя, умножается на соответствующий ему вес. 

**Расчет и сравнение с общей точностью на примере из трех результатов:**
1) Результат 95% с весом 100%
2) Результат 85% с весом 95%
3) Результат 60% с весом 90.25%

| Шаг вычислений | Общая точность | Взвешенная общая точность |
| ---------------| -------------- | ------------------- |
| Сумма точностей результатов | s = 95 + 85 + 60 = 240 (простое сложение) | s<sub>w</sub> = 95 + 85×0.95 + 60×0.9025 = 229.9 (каждый результат взвешивается) |
| Идеальная точность | m = 100 × 3 = 300 (т.к. идеал везде 100%) | m<sub>w</sub> = 100 + 100×0.95 + 100×0.9025 = 285.25 (каждый идеал взвешивается) |
| Результат | a = s/m = 240/300 = 0.8 или 80% | a<sub>w</sub> = s<sub>w</sub>/m<sub>w</sub> = 229.9/285.25 0.8059 или 80.59% |
